...
function getJobs (printer, req, res) {
  ...
  var _groups = printer.jobs
    .filter(function (job) {
      switch (which) {
        case 'completed':
          return ~[C.JOB_COMPLETED, C.JOB_CANCELED, C.JOB_ABORTED].indexOf(job.state)
        case 'not-completed':
          return ~[C.JOB_PENDING, C.JOB_PROCESSING, C.JOB_PROCESSING_STOPPED, C.JOB_PENDING_HELD].indexOf(job.state)
        default:
          return true
      }
    })
