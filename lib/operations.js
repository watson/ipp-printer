...
function getPrinterAttributes (printer, req, res) {
  var group = groups.unsupportedAttributesTagPrinter(printer, req._body)
  var _groups = group.attributes.length > 0 ? [group] : []
  _groups.push(groups.printerAttributesTag(printer, req._body))
  res.send(_groups)
}
...
function getJobs (printer, req, res) {
  ...
  var group = groups.unsupportedAttributesTagJob(req._body)
  var _groups = group.attributes.length > 0 ? [group] : []
