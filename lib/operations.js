...
function getPrinterAttributes (printer, req, res) {
  res.send([
    groups.unsupportedAttributesTagPrinter(printer, req._body),
    groups.printerAttributesTag(printer, req._body)
  ])
}
...
function getJobs (printer, req, res) {
  ...
  var requestedAttributes = utils.requestedAttributes(req._body) || ['job-uri', 'job-id']
  ...
  res.send([groups.unsupportedAttributesTagJob(req._body)].concat(_groups))
}
...
function getJobAttributes (printer, req, res) {
  ...
  res.send([
    groups.unsupportedAttributesTagJob(req._body),
    {
      tag: C.JOB_ATTRIBUTES_TAG,
      attributes: job.getAttributes(utils.requestedAttributes(req._body))
    }
  ])
}
